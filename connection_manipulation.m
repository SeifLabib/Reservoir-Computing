...
%%% Modify the selected elements by 5% of their initial values
nonZeroIndices = find(M);
random_indices = randsample(nonZeroIndices, 30);

signe = sign(randn);
for i = 1:length(random_indices)
    idx = random_indices(i);
    M(idx) = M(idx) * signe*(1.0 + 0.05*j); % Increase by 5%
end
...
title('Pattern similarity vs weights modification percentage', 'fontsize', fontsize-5, 'fontweight', fontweight);
xlabel('Weight modification percentage', 'fontsize', fontsize-5, 'fontweight', fontweight);
hold on;
